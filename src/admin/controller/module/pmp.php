...
namespace Opencart\Admin\Controller\Extension\Pmp\Module;
class PMP extends \Opencart\System\Engine\Controller {

	public 	$_route 		= 'extension/pmp/module/pmp';
	public 	$_model 		= 'model_extension_pmp_module_pmp';
	private $_version 		= '1.2';

	private $_events = [
		[
			'trigger'	=> 'catalog/controller/extension/opencart/module/featured/before',
			'action'	=> '|eventPMPFeaturedAddData'
		],
		[
			'trigger'	=> 'catalog/view/extension/opencart/module/featured/before',
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('pmp.pmp', $this->request->post);
		...
		if (!isset($this->request->get['module_id'])) {
			...
			$data['action'] = $this->url->link($this->_route, 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		$this->load->model('setting/module');
		$data['modules'] = $this->model_setting_module->getModulesByCode('pmp.pmp');
		...
		$grps = glob(DIR_EXTENSION . '/pmp/admin/controller/module/pmp/groups/*.php', GLOB_BRACE);

		foreach ($grps as $file) {
			$group = basename($file, '.php');
			$datasources = [];

			$algs = glob(DIR_EXTENSION . '/pmp/admin/controller/module/pmp/datasources/' . $group . '_*.php', GLOB_BRACE);

			foreach ($algs as $file) {
				$datasource = basename($file, '.php');

				$this->load->language('extension/pmp/module/pmp/datasources/' . $datasource, 'extension');
			...
			$this->load->language('extension/pmp/module/pmp/groups/' . $group, 'extension');
		...
		$sections = glob(DIR_EXTENSION . '/pmp/admin/controller/module/pmp/sections/*.php', GLOB_BRACE);

		foreach ($sections as $file) {
			$section = basename($file, '.php');

			$this->load->language('extension/pmp/module/pmp/sections/' . $section, 'extension');

			$data['sections'][] = [
				'code' => $section,
				'text' => $this->language->get('extension_heading_title'),
				'icon' => $this->language->get('extension_fontawesome_icon'),
		...
		if (isset($this->request->post['template'])) {
			...
			$data['template'] = 'extension/pmp/module/pmp';
		}

		if (isset($this->request->get['module_id']) && isset($module_info['data_source'])) {
			$data['data_source_form_data'] = $this->load->controller('extension/pmp/module/pmp/datasources/' . $module_info['data_source'] . '|getForm', (int) $this->request->get['module_id']);
		} else {
			$data['data_source_form_data'] = $this->load->controller('extension/pmp/module/pmp/datasources/' . $data['data_source'] . '|getForm', 0);
		}
	...
	public function data_source() {
		if (($this->request->server['REQUEST_METHOD'] == 'GET')) {
			...
			$response = $this->load->controller('extension/pmp/module/pmp/datasources/' . $datasource . '|getForm', $module_id);
		} else {
			$response = '';
		}
