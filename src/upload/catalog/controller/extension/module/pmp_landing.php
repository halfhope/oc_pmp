...
class ControllerExtensionModulePMPLanding extends Controller {
	...
	public function index() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link($this->_route, $url)
		);
		...
		$data['sorts'][] = array(
			...
			'href'  => $this->url->link($this->_route, 'sort=p.sort_order&order=ASC' . $url)
		);

		$data['sorts'][] = array(
			'text'  => $this->language->get('text_name_asc'),
			'value' => 'pd.name-ASC',
			'href'  => $this->url->link($this->_route, 'sort=pd.name&order=ASC' . $url)
		);

		$data['sorts'][] = array(
			'text'  => $this->language->get('text_name_desc'),
			'value' => 'pd.name-DESC',
			'href'  => $this->url->link($this->_route, 'sort=pd.name&order=DESC' . $url)
		);

		$data['sorts'][] = array(
			'text'  => $this->language->get('text_price_asc'),
			'value' => 'ps.price-ASC',
			'href'  => $this->url->link($this->_route, 'sort=ps.price&order=ASC' . $url)
		);

		$data['sorts'][] = array(
			'text'  => $this->language->get('text_price_desc'),
			'value' => 'ps.price-DESC',
			'href'  => $this->url->link($this->_route, 'sort=ps.price&order=DESC' . $url)
		);
		...
		if ($this->config->get('config_review_status')) {
			$data['sorts'][] = array(
				...
				'value' => 'rating-DESC',
				'href'  => $this->url->link($this->_route, 'sort=rating&order=DESC' . $url)
			);

			$data['sorts'][] = array(
				'text'  => $this->language->get('text_rating_asc'),
				'value' => 'rating-ASC',
				'href'  => $this->url->link($this->_route, 'sort=rating&order=ASC' . $url)
			);
		}
		...
		$data['sorts'][] = array(
				...
				'value' => 'p.model-ASC',
				'href'  => $this->url->link($this->_route, 'sort=p.model&order=ASC' . $url)
		);

		$data['sorts'][] = array(
			'text'  => $this->language->get('text_model_desc'),
			'value' => 'p.model-DESC',
			'href'  => $this->url->link($this->_route, 'sort=p.model&order=DESC' . $url)
		);
        ...
        if (!$this->config->get('config_canonical_method')) {
            ...
            if ($page == 1) {
                $this->document->addLink($this->url->link($this->_route, '', true), 'canonical');
            } elseif ($page == 2) {
                $this->document->addLink($this->url->link($this->_route, '', true), 'prev');
            } else {
                $this->document->addLink($this->url->link($this->_route, 'page=' . ($page - 1), true), 'prev');
            }

            if ($limit && ceil($product_total / $limit) > $page) {
                $this->document->addLink($this->url->link($this->_route, 'page=' . ($page + 1), true), 'next');
            }
        } else {
            ...
            $canonical_url = $this->url->link($this->_route, '', true);
